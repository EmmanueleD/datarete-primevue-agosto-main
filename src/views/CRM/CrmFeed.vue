<template>
    ...
    <TabView ref="tabview1" v-model:activeIndex="active">
      <TabPanel header="Anagrafica">
        <NuovaAnagrafica
          ...
          :showContatti="false"
          :showDocumenti="false"
          :sidebarData="{ ID: route.params.idAnagrafica }"
        ></NuovaAnagrafica>
      </TabPanel>
        ...
        <Card class="mb-2" v-for="item of tutteLeAttivita" :key="item.id">
          <template #content>
            <CrmItemNota
              ...
              v-if="item.id_type == 1 || item.id_type == 0"
              :item="item"
            >
            </CrmItemNota>
            <CrmItemAppuntamento
      ...
      <TabPanel header="Attività">
        <div class="w-full flex flex-column mb-4 p-2">
          <div class="w-full flex justify-content-end mb-4 p-2">
            <Button
              @click="showSidebarEventi(currentProfile)"
        ...
        <div class="w-full flex flex-column">
          <div class="w-full flex justify-content-end">
            <SplitButton
              ...
              icon="pi pi-plus"
              :model="buttonNuovoPreventivo"
            ></SplitButton>
          </div>
        </div>

        <Preventivi
          :idAnagrafica="route.params.idAnagrafica"
          :headerVisible="false"
          :searchVisible="false"
        >
        </Preventivi>
      ...
      <TabPanel header="Altri Impegni">
        ...
        >
          <Column header="Azioni">
            <template #body="{ data }">
              <Button
                @click="showSidebarAltriImpegni(data)"
                icon="pi pi-pencil"
                class="p-button-rounded mr-2"
              />
              <Button
                @click="confirmDeleteAltriImpegni(data)"
                icon="pi pi-trash"
        ...
        >
          <div class="wrapper">
            ...
            >
              <InputSwitch
                v-model="sidebarAltriImpegniData.erogato"
              ></InputSwitch>
              <span>Erogato</span>
            </div>
            <div class="flex w-full justify-content-between mb-4 flex-column">
            ...
            >
              <InputSwitch
                v-model="sidebarAltriImpegniData.estingue"
              ></InputSwitch>
              <span>Estingue</span>
            </div>
            <div class="flex w-full justify-content-between mb-4 flex-column">
  ...
  >
    <SidebarEventi
      @event_refreshEvents="resetSidebarEventi"
      :sidebarData="sidebarEventiData"
    ></SidebarEventi>
  </Sidebar>
</template>
...
import PraticheUtente from "../pratiche/PraticheUtente.vue"
...
function getAltriImpegni() {
  ...
  const service = new AxiosService(
    "Anagrafiche/GetAltriImpegni/" + route.params.idAnagrafica
  )
  service
    .read()
    .then((res) => {
...
const buttonNuovoPreventivo = [
  {
    ...
    command: () => {
      router.push(
        "/pratiche/preventivo-prestito/" + route.params.idAnagrafica + "/0"
      )
    },
  },
  {
    ...
    command: () => {
      router.push(
        "/pratiche/preventivo-mutuo/" + route.params.idAnagrafica + "/0"
      )
    },
  },
  {
    ...
    command: () => {
      router.push(
        "/pratiche/preventivo-cqs/" + route.params.idAnagrafica + "/0"
      )
    },
  },
  {
    ...
    command: () => {
      router.push(
        "/pratiche/preventivo-delega/" + route.params.idAnagrafica + "/0"
      )
    },
  },
]
...
function saveAltriImpegni() {
  ...
  if (sidebarAltriImpegniData.value.ID) {
    ...
    service
      .update(sidebarAltriImpegniData.value)
      .then((res) => res)
      .finally(() => {
    ...
    const service = new AxiosService(
      "Anagrafiche/AddAltriImpegni/" + route.params.idAnagrafica
    )
    service
      .create(sidebarAltriImpegniData.value)
      .then((res) => res)
      .finally(() => {
        resetAltriImegnisideBar()
        loading.value = false
...
function getCrmItems() {
  ...
  serviceGET
    .readCustomEndpoint("Crm/GetCrmRecord/" + route.params.idAnagrafica, "")
    .then((res) => {
...
function deleteItem(item) {
  const serviceDELETE = new AxiosService(
    "Crm/DeleteCrmRecord/" + route.params.isAnagrafica
  )
  serviceDELETE
    .deleteCustomWQuery(
      "Crm/DeleteCrmRecord/" + route.params.isAnagrafica,
...
function getEvents(event) {
  console.log(
    "🚀 ~ file: PrenotazioneSale.vue ~ line 159 ~ getEvents ~ event",
    event
  )
  ...
  if (
    ...
    const service = new AxiosService(
      "Calendar/GetEvents/" + route.params.idAnagrafica + "/0"
    )
