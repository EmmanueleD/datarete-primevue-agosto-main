<template>
  <div class="flex">
    <div class=" flex-grow-1 flex align-items-center">
      <i class="pi pi-fw pi-calendar mr-2 pt-2"></i>
      <h4>{{ item.titolo }}</h4>
    </div>
  ...
  <Sidebar class="p-sidebar-md" @hide="hideSidebar" v-model:visible="sidebarVisible" :baseZIndex="10000"
    position="right">
    <CrmPostPutAppuntamento :item="item"></CrmPostPutAppuntamento>
  </Sidebar>
</template>

<script setup>
import { ref } from 'vue';
import CrmPostPutAppuntamento from './CrmPostPutAppuntamento.vue';
