<template>
      ...
      <template #content>
        ...
        <DataTable v-else :value="data" :stripedRows="true" :paginator="true" :rows="10" :resizableColumns="true"
          ...
          <Column header="Azioni" style="max-width: 200px">
            <template #body="{ data }">
              <Button icon="pi pi-external-link" class="p-button-rounded p-button-outlined"></Button>
            </template>
          </Column>
          <Column field="nomeCliente" header="Cliente"></Column>
          <Column field="agente" header="Agente"></Column>
          <Column field="tipo_preventivatore" header="Prodotto"></Column>
          <Column field="importoRichiesto" header="Importo richiesto"></Column>
          <Column field="importoRata" header="Rata"></Column>
          <Column field="dataCreazione" header="Data creazione">
            <template #body="{ data }">
              <span v-if="data.dataCreazione">{{ new Date(data.dataCreazione).toLocaleDateString('it') }} - {{ new
                  Date(data.dataCreazione).toLocaleTimeString('it', { hour: '2-digit', minute: '2-digit' })
              }}</span>
            </template>
          </Column>
          <Column field="numeroRisultati" header="N risultati"></Column>
  ...
  <!-- <Sidebar v-model:visible="sidebarVisible" :baseZIndex="10000" position="right" class="p-sidebar-md"
    ...
    <sbElencoPreventiviSalvati :sidebarData="sidebarData">
    </sbElencoPreventiviSalvati>
  </Sidebar> -->
</template>
...
// import sbElencoPreventiviSalvati from '@/components/sidebars/sbElencoPreventiviSalvati.vue';
...
const view = {
  ...
  endpointGET: 'Preventivatore/GetPreventivi',
