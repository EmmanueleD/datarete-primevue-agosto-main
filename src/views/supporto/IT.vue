<template>
    ...
    <Card>
      <template #content>
        <DataTable :value="data" :stripedRows="true" :paginator="true" :rows="10" :resizableColumns="true"
          ...
          <Column header="ULTIMO MESSAGGIO" field="ultimo_messaggio"></Column>
  ...
  <Sidebar class="p-sidebar-md" @hide="hidePPTicket" v-model:visible="sidebarPPTicketVisible" :baseZIndex="10000"
    position="right">
    <PostPutTicket @hide_newTicket="hideNewTicket" :id_type="1"></PostPutTicket>
  </Sidebar>
</template>
...
function getViewData() {
  ...
  serviceGET.read()
    .then(res => {
      ...
      // Supporto IT - id_type == 1
      data.value = res.filter(x => x.id_type == 1)
    })
}
