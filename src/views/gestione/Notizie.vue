<template>
    ...
    <Card>
      <template #content>
        ...
        <DataTable v-else :value="data" :stripedRows="true" :paginator="true" :rows="10" :resizableColumns="true"
          ...
          <Column field="data" header="Data Creazione">
            <template #body="{data}">
              {{ new Date(data.data).toLocaleDateString('it', {
  ...
  <Sidebar v-model:visible="sidebarVisible" :baseZIndex="10000" position="right" class="p-sidebar-md"
    ...
    <sbNotizie @event_refreshList="hideSidebar" :sidebarData="sidebarData">
    </sbNotizie>
  </Sidebar>
</template>
...
import sbNotizie from "@/components/sidebars/sbNotizie.vue"
...
const view = {
  title: 'Notizie',
  endpointGET: 'News/GetList',
  endpointPOST: 'News/Insert',
  endpointPUT: 'News/Edit', // /ID
  endpointDELETE: 'News/Delete' // /ID
}
...
function showSidebar(event) {
  ...
  sidebarData.value.event.data = new Date()
...
function confirmDelete(element) {
  confirm.require({
    message: 'Sei sicuro di voler eliminare "' + element.titolo + '"',
