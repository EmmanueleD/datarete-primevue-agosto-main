    ...
    <Card>
      <template #content>
        ...
        <DataTable v-else :value="data" :stripedRows="true" :paginator="true" :rows="10" :resizableColumns="true"
          ...
          <Column field="TipoSede" header="TIPO">
            <template #body="{data}">
              {{ data.TipoSede }}
            </template>
          </Column>
          ...
          <Column field="Regione" header="PROVINCIA">
            <template #body="{data}">
              {{ data.Regione }}
            </template>
          </Column>
          <Column field="Citta" header="CITTA">
            <template #body="{data}">
              {{ data.Citta }}
            </template>
          </Column>
...
// SETTING VIEW
const view = {
  title: 'Gestione Sedi Azienda',
  endpointGET: 'Gestione/SediAzienda/GetSedi', //ID
  endpointGETwID: 'Gestione/SediAzienda/GetSede', //ID
  endpointPOST: 'Gestione/SediAzienda/AddSede/',
  endpointPUT: 'Gestione/SediAzienda/UdateSede', // /ID
  endpointDELETE: 'Gestione/SediAzienda/DeleteSede', // /ID
}
