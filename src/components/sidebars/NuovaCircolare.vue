<template>
  ...
  <div class="grid w-full">
    ...
    <div class="flex flex-column col-12  mb-4">
      ...
      <InputText id="argomento" type="text" v-model="tmpItem.argomento"></InputText>
    </div>
  ...
  <FileUpload @uploader="uploadFile" mode="basic" uploadIcon="pi pi-paperclip" name="demo[]" :customUpload="true"
    :previewWidth="50" :maxFileSize="1000000"
    :chooseLabel="tmpItem.urlFile ? fileName(tmpItem.urlFile) : 'Allega File Circolare'" :auto='true' />
  <div class="w-100 flex justify-content-end align-items-end col-12 col-md-2">
    <Button label="Salva Circolare" @click="inviaCircolare" :loading="loading"></Button>
  </div>
</template>
<script setup>
import { ref } from 'vue';
...
function uploadFile(ev) {
  ...
  for (let i = 0; i < ev.files.length; i++) {
    ...
    service.postCustomEndpoint('Upload?type=' + 'Circolari', '', formData)
      .then(res => {
        console.log("🚀 ~ file: NuovaCircolare.vue ~ line 78 ~ uploadFile ~ res", res)
...
function inviaCircolare() {
  ...
  if (tmpItem.value.id) {
    ...
    servicePUT.update(tmpItem.value)
      .then(res => {
        if (res) {
          toast.add({ severity: 'success', summary: 'Nuova Opzione Creata', detail: tmpItem.value.titolo, life: 3000 });
    ...
    servicePOST.postCustomEndpoint('Circolari/AddCircolare', '', newCircolare.value)
      .then(res => {
        if (res) {
          toast.add({ severity: 'success', summary: 'Nuova Circolare Inviata', detail: tmpItem.value.titolo, life: 3000 });
      ...
      .catch(error => {
        toast.add({ severity: 'error', summary: "'Errore nell'invio della Circolare'", detail: error, life: 3000 });
...
function getLivelli() {
  serviceLivelliGET.read().then(res => {
    console.log("🚀 ~ file: NuovaCircolare.vue ~ line 156 ~ serviceLivelliGET.read ~ res", res)
