<template>
      ...
      <template #content>
        ...
        <DataTable v-else :value="data" :stripedRows="true" :paginator="true" :rows="10" :resizableColumns="true"
          ...
          <Column v-for="col of columns" :field="col.field" :header="col.header" :key="col.field"
            ...
            <template #body="{ data }">
              <div v-if="col.field == 'testo'" class="grid-center w-full">
                <!-- {{ data.testo.splice(0, 10) }}... -->
                {{ data.testo.slice(0, 10) }}...
              </div>
              <div v-else>
                {{ data[col.field] }}
              </div>
  ...
  <Sidebar v-model:visible="sidebarVisible" :baseZIndex="10000" position="right" class="p-sidebar-md"
    ...
    <sbModelliMail :sidebarData="sidebarData">
    </sbModelliMail>
  </Sidebar>
</template>
...
import sbModelliMail from '@/components/sidebars/sbModelliMail.vue';
...
const data = ref([
  {
    ...
    categoria: 'pratiche',
    nome: 'modello uno',
    testo: 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Curabitur ac venenatis sem. Cras quis posuere.'
  }
])

const columns = [
  { field: 'categoria', header: 'CATEGORIA', sortable: false },
  { field: 'nome', header: 'NOME', sortable: true },
  { field: 'testo', header: 'TESTO', sortable: true },
]
