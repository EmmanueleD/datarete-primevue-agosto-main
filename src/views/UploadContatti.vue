  ...
  <div class="wrapper">
      ...
      <template #content>
        <div class="flex gap-4 justify-content-between align-items-center">
          ...
          <FileUpload @uploader="uploadFile" mode="basic" uploadIcon="pi pi-paperclip" name="demo[]"
            :customUpload="true" :previewWidth="50" :maxFileSize="1000000" chooseLabel="Upload" :auto='true'
            :loading="loading" />
        </div>
      </template>
    </Card>
    ...
    <Card v-if="autorizzato">
      <template #content>
        <div class="flex gap-4 justify-content-between align-items-center">
          <div class="flex align-items-center mb-4">
            <label>Importatore contatti</label>
          </div>
          <i class="pi-spin pi spinner" v-if="loading"></i>
          <FileUpload @uploader="uploadFile2" mode="basic" uploadIcon="pi pi-paperclip" name="demo[]"
            :customUpload="true" :previewWidth="50" :maxFileSize="9000000000" chooseLabel="Upload" :auto='true'
            :loading="loading" />
        </div>
      </template>
    </Card>
...
function uploadFile(ev) {
  ...
  for (let i = 0; i < ev.files.length; i++) {
    ...
    service.postCustomEndpoint('Upload?type=' + nomeCartella.value, '', formData)
      .then(res => {
...
function uploadFile2(ev) {
  ...
  for (let i = 0; i < ev.files.length; i++) {
    ...
    service.postCustomEndpoint('Upload?type=' + nomeCartella.value, '', formData)
      .then(res => {
        fileToUpload.value = res
        const serviceImportanzione = new AxiosService('Import/ImportExcelContatti')
        serviceImportanzione.create({ filePath: fileToUpload.value })
          .then(res => {
            if (res) {
