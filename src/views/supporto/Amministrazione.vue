<template>
  <div class="wrapper">
    <h1>Supporto Amministrazione</h1>
    <Card class="mb-4">
      <template #content>
          ...
          <Column header="STATO" field="status_text">
            <template #body="{ data }">
              <div class="flex flex-column align-items-center w-100">
                <i
                  v-if="data.id_status == 0"
                  style="color: dimgray"
                  class="pi pi-fw pi-circle-fill"
                ></i>
                <i
                ...
                <i
                  ...
                  class="pi pi-fw pi-circle-fill"
                ></i>
                <span style="font-size: 0.8rem">{{ data.status_text }}</span>
              </div>
            </template>
          </Column>
          <Column header="CHAT">
  ...
  <Sidebar
    ...
    :baseZIndex="10000"
    position="right"
    class="p-sidebar-md chat"
    @hide="hideSidebar"
  >
    <template #header>
      <div class="flex justify-content-start align-items-center">
        <Avatar
          v-if="chatData.created_by_avatar"
          :image="chatData.created_by_avatar"
          size="large"
          class="mr-4"
        >
        </Avatar>
  ...
  <Sidebar
    class="p-sidebar-md"
    @hide="hidePPTicket"
    v-model:visible="sidebarPPTicketVisible"
    :baseZIndex="10000"
    position="right"
  >
    <PostPutTicket @hide_newTicket="hideNewTicket" :id_type="4"></PostPutTicket>
  </Sidebar>
</template>
...
function getViewData() {
  ...
  serviceGET.read().then((res) => {
    ...
    data.value = res.filter((x) => x.id_type == 4)
  })
}
