  ...
  <div>
    <div class="flex justify-content-between align-items-center w-full mb-4">
      <div class="flex flex-column mb-4">
        ...
        <div class="flex flex-column mb-4">
          <label for="questionario-title">Categoria</label>
          <Dropdown
            :options="[
              { value: 1, label: 'CRM' },
              { value: 2, label: 'Pratiche' },
            ]"
            optionLabel="label"
            optionValue="value"
            v-model="tmpQuestionario.id_tipo"
          ></Dropdown>
        </div>
      </div>
      <Button
    ...
    <template
      v-for="(question, index) of tmpQuestionario.questions"
      :key="index"
    >
      <PostPutDomanda
        v-if="!question.delete"
        @deleteDomanda="deleteDomanda"
        @eventEditQuestion="eventEditQuestion"
        :question="question"
        :questionIndex="index"
      >
      </PostPutDomanda>
